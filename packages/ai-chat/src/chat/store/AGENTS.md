...
- **Imports from `redux` / `react-redux`** — those packages are intentionally not deps. Use the local `useSelector`; dispatch through `serviceManager.store.dispatch` with an action creator from `actions.ts`.
