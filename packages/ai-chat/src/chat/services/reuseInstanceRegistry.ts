 ...
 * unmount/remount when `next.reuseInstance` is enabled. It lives outside React (a plain module
...
interface ReuseInstanceEntry {
  ...
  /** Whether the accidental-reboot diagnostic has already fired for this namespace. */
  rebootWarned: boolean;
}
 ...
 * transitions. Tunable per chat via `next.reuseInstanceGraceMs`.
 */
...
/**
 * Returns the entry for a namespace without changing its ref-count. Used by the accidental-reboot
 * diagnostic to read/set `rebootWarned`.
 */
...
export function registerServiceManager(
  ...
  registry.set(key, {
    ...
    refCount: 1,
    graceTimer: null,
    rebootWarned: existing?.rebootWarned ?? false,
  });
}
