...
function createServiceManager(appConfig: AppConfig) {
  ...
  // Capture every store-subscription handle so `unloadServices` can tear them down on
  // disposal. There are exactly four here: `copyToSessionStorage`, `fireStateChangeEvent`,
  // `refreshLocalizationOnChange`, and the theme-diff watcher below. A fifth is registered
  // later, in `ChatSDK.ts`'s `acquireChatSDK`, by `attachMessagesStateTracking`.
