...
interface LauncherProps {
  ...
  extended: boolean;
  wasReduced: boolean;
  setLauncherWasReduced: (value: boolean) => void;
...
function Launcher(props: LauncherProps) {
  ...
  // tooling preview.
  const playExtendAnimation = prevExtended === undefined && !extended;
  // Indicates if the launcher is playing the "extend" animation.
  const isExtending = prevExtended !== undefined && !prevExtended && extended;
  ...
  const disableIntroAnimation = isExtending || hasReduced;
  ...
  const setLauncherStateAsReduced = useCallback(() => {
    if (!wasReduced) {
      setLauncherWasReduced(true);
    }
  }, [setLauncherWasReduced, wasReduced]);
  ...
  }, [reduceLauncher, setLauncherStateAsReduced]);
  ...
  const ariaLabelSuffix =
    unreadHumanAgentCount !== 0
      ? formatUnreadMessageLabel?.({ count: unreadHumanAgentCount })
      : undefined;
  ...
  return (
    ...
    >
      <Button
        ...
        tooltipPosition={BUTTON_TOOLTIP_POSITION.LEFT}
