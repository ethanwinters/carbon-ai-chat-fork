...
The rounding values come from these tokens, so use them in your own CSS when you are not using `data-rounded`:
...
Cards map `--cds-aichat-card-border-radius` to `--cds-aichat-rounded-modifier-radius`, so slotted content inside a card can use the same modifiers and stay visually aligned.
