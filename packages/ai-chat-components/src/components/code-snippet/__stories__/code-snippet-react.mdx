...
import * as CodeSnippetReactStories from "./code-snippet-react.stories.jsx";
...
The `<CodeSnippet>` and `<CodeSnippetTileContainer>` components wrap the `cds-aichat-code-snippet` web component to provide syntax highlighting, streaming updates, and editable code blocks inside React applications.
...
import CodeSnippetTileContainer from "@carbon/ai-chat-components/es/react/code-snippet-tile-container.js";

function ReadOnlySnippet({ code }) {
  return <CodeSnippet highlight wrapText>{code}</CodeSnippet>;
}
```
...
### `<CodeSnippetTileContainer>` props
...
function StreamingSnippet({ stream }) {
  ...
  return (
    <CodeSnippetTileContainer highlight wrapText>
      {content}
