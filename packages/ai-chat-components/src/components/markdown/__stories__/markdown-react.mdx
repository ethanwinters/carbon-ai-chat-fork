...
function MarkdownExample({ content }) {
  return <Markdown highlight markdown={content} />;
}
```
...
**Performance Note:** The `markdown` prop is more efficient than children because it bypasses the slot change detection mechanism. This is particularly beneficial when streaming markdown content, as it reduces unnecessary DOM operations and improves rendering performance.
...
Use streaming mode to gradually reveal markdown content. The `markdown` prop is recommended for optimal performance:
