...
import * as MarkdownTextStories from "./markdown-text.stories";

<Meta of={MarkdownTextStories} />

# AI Chat Markdown Text

The `cds-aichat-markdown-text` component renders markdown content with syntax highlighting, tables, code snippets, and streaming support. It's optimized for displaying AI-generated responses with rich formatting.
...
<Markdown>{`${cdnJs(["markdown-text"])}`}</Markdown>
...
<Canvas of={MarkdownTextStories.Default} />

#### Streaming content

<Canvas of={MarkdownTextStories.Streaming} />

#### HTML sanitization

<Canvas of={MarkdownTextStories.WithHTMLSanitization} />

## `<cds-aichat-markdown-text>` attributes, properties and events

<ArgTypes of={MarkdownTextStories.Default} />
...
```javascript
const markdown = document.querySelector('cds-aichat-markdown-text');
