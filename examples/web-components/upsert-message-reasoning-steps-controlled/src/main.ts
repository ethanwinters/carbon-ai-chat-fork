...
/**
 * Example: Carbon AI Chat — Upsert message / Reasoning steps (controlled) (Web components)
 *
 ...
 * even though every individual step inside is marked `OPEN` — all delivered
 * through `upsertMessage` instead of the chunk-based streaming API.
 *
 * APIs exercised:
 *   - `<cds-aichat-custom-element>`
 *   - `instance.messaging.upsertMessage` (reasoning updates)
...
const config: PublicConfig = {
  messaging: {
    ...
    // reasoning updates via upsertMessage without a backend.
    customSendMessage,
  },
  layout: {
