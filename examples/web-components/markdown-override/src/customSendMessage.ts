 ...
 * plus a markdown table (rendered through a Carbon `cds-table` by the `table`
 * override):
 *
 ...
 *      the line count.
 *   3. A markdown table — replaced by a Carbon `cds-table`.
 */
...
The reply also includes a markdown table, rendered through a Carbon \`cds-table\` by the \`table\` custom renderer:
