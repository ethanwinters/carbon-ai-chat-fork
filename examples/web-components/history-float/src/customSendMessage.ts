...
/**
 * Mock `customSendMessage` for the chat history (float) example.
 *
 * Demonstrates: a fully client-side back-end that returns either a
 * single `addMessage` payload or a streamed response via
 * `addMessageChunk`, including stop/abort handling.
 *
 * APIs exercised:
...
// Simulates a streamed assistant reply by emitting partial text chunks on a fixed cadence. Replace with a real production implementation that pipes back-end stream events into `addMessageChunk`.
async function doFakeTextStreaming(
  ...
  // Aborts pending word emissions when the user clicks stop or the chat is reset, so the simulated stream cleans up promptly.
  const abortHandler = () => {
    isCanceled = true;
    // Cancels every queued word so no late chunks leak into the UI after abort.
  ...
  try {
    ...
    if (!isCanceled) {
      ...
      // Emits a truncated `stream_stopped` payload so the chat reconciles a partial answer when streaming is aborted mid-flight.
      const completeItem = {
        response_type: MessageResponseTypes.TEXT,
...
// Mock back-end entry point routing the user's text to a canned response. Replace with a real production implementation backed by your service.
async function customSendMessage(
  request: MessageRequest,
  requestOptions: CustomSendMessageOptions,
  instance: ChatInstance,
) {
  // Empty text is treated as the chat opening event, so respond with the welcome message that lists supported demo commands.
