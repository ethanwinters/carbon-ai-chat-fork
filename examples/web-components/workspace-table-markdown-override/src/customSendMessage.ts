...
const REPLY = `Here are recent orders. The default markdown renderer for tables is replaced with a card-wrapped preview — click the maximize icon in the card's header to open the same data in the workspace panel, where it renders as a full Carbon \`<cds-table>\` with all rows visible and the workspace shell body providing a vertical scrollbar.
