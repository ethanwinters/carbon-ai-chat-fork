...
/**
 * Mock back-end for the chat-history-float example.
 *
 * Demonstrates: a fully client-side `customSendMessage` implementation that
 * routes hard-coded inputs (`text`, `stream text`) to either a single
 * `addMessage` call or a chunked streaming flow, while honoring the abort
 * signal so the stop button and conversation resets cleanly cancel work.
 *
 * APIs exercised:
...
// Replace with a real production implementation that streams from your model
// or service.
async function doFakeTextStreaming(
  ...
  // The chat aborts the signal both when the user clicks stop and when the
  // conversation is cleared, so cancellation must drop every queued chunk to
  // avoid emitting tokens into a fresh conversation.
  const abortHandler = () => {
    isCanceled = true;
...
// Replace with a real production implementation that forwards `request` to
// your back-end and pipes results back through `instance.messaging`.
