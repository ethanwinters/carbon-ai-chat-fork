...
/**
 * Mock backend for the chat-history-fullscreen example.
 *
 * Demonstrates: a fully client-side `customSendMessage` that returns a
 * welcome card, a static text response, or a word-by-word streamed
 * response with abort-signal support.
 *
 * APIs exercised:
 *   - `PublicConfig.messaging.customSendMessage`
 *   - `instance.messaging.addMessage`, `instance.messaging.addMessageChunk`
...
async function doFakeTextStreaming(
  ...
  // the AbortSignal fires for both the user's stop button and a programmatic restart/clear, so a single handler must cancel any in-flight chunk emission and pending timers.
  const abortHandler = () => {
    isCanceled = true;
  ...
  try {
    ...
    if (!isCanceled) {
      ...
      // on cancellation, send a final chunk with `stream_stopped: true` so the chat UI commits a partial transcript instead of leaving a dangling streaming bubble.
      const completeItem = {
        response_type: MessageResponseTypes.TEXT,
