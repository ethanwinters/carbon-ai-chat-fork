...
- **State the proposal as a claim, not a history.** "The SDK ships as an entry point in `@carbon/ai-chat`," not "after considering several packaging options…". What was considered belongs in Alternatives.
- **Claim before scaffolding.** Summary and Proposal carry the ask. Together they stay larger than Motivation and Alternatives.
- **Show code before prose in Proposal and Consumer impact.** A sample of what a host writes carries a surface faster than a paragraph about it. Words carry only what the code can't show.
