...
- Node ≥ 22, npm ≥ 10.
- **Never read or edit generated output**: `dist/`, `es/`, `es-custom/`, `storybook-static/`, `storybook-react-static/`, `node_modules/`, `packages/ai-chat-components/src/react/`, `packages/ai-chat-components/custom-elements.json`, `packages/ai-chat/dist/docs/`, `packages/ai-chat/docs/api/`, `telemetry.yml`. Regenerate via the documented commands (`npm run custom-elements`, `npm run telemetry:config`). `packages/ai-chat/docs/api/` is checked in but **regenerated at release, not per PR** → [docs/AGENTS.md](packages/ai-chat/docs/AGENTS.md#the-checked-in-api-snapshot).
