...
The headless SDK in ADR-0002 uses this same model, and that record covers the SDK's own shape. Either record reads on its own.
...
```ts
if (instance.state.get().messages.length === 0) {
  seedWelcome();
}
```
...
- **Saved session state** covers view state, the home screen, the launcher, the unread indicator, accepted disclaimers, and human-agent profiles and connection state. When your app owns that storage — through `persistedState` in a shell, or `humanAgentState` in a composed chat — the chat can't reach it, so `destroy()` reports the cleared value to your `onStateChange` instead. Save what you're given, and a destroyed conversation doesn't come back on the next boot. A window that runs out reports nothing, so your copy stays as it is and the next mount reconnects from it.
...
- **Other handles to a destroyed chat stop working.** Methods that return a promise reject, and the rest do nothing. Their `state` keeps its last value and stops notifying. The persistence callback is not a `state` subscription: it fires once during shutdown, after `state` has stopped notifying, so a host is told to drop its saved copy.
...
```ts
/**
 * Lifetime settings on `ChatSDKConfig`, so the SDK and the shells both take them.
 *
 * @category SDK
 */
interface ChatSDKLifecycleConfig {
  /**
   * How long a released chat stays alive, waiting to be adopted, before it shuts down.
   * Defaults to 3000. Raise it when your users leave the chat and come back, such as
   * while a slow route loads.
   */
  releaseGraceMs?: number;
}

/**
 * What the shells add to it. `PublicConfig` carries this shape. ADR-0002 sets the rule
 * the two names follow: the SDK owns the prefixed type, and the shells' type extends it.
 */
interface LifecycleConfig extends ChatSDKLifecycleConfig {
  /**
   * What unmounting does: `'release'`, the default, which keeps the conversation alive
   * for the grace window, or `'destroy'`, which ends it and clears saved session state.
   * Use `'destroy'` where a remount must start fresh, such as logout.
   *
   * A web component takes it through its `lifecycle` property, or as a JSON `lifecycle`
   * attribute.
   */
  onUnmount?: 'release' | 'destroy';

  /**
   * Keeps the conversation alive across a remount. Defaults to false.
   *
   * 1.x only. In 2.0.0 reuse is how every chat works, this key is removed, and a config
   * that still sets it fails to compile.
   */
