...
So the entry point is mostly a question of what to remove and what to hand back. The one genuinely new thing is lifecycle. No `render()`, `destroy()`, or `release()` exists on the instance today; the only teardown verb is `destroySession`, which clears a session rather than disposing anything. A headless host has no component to unmount, so it needs a way to say it is finished — and the shells need a way to push config changes that today they apply through an internal path.
...
The view half is absent by construction, not by omission. A headless host has no panels, no input, and no scroll, so the members that drive them are not on this surface — including their state stores. What it does have is the whole conversation: sending, receiving, history, human agents, events, and conversation state.
