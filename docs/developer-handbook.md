...
| `npm run measure -- --changed <base>` | Runs those three over a diff, then adds what only a diff shows: each new file's size beside its neighbors, and what the added lines contain |
